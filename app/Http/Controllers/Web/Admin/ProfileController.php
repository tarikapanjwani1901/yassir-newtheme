...
class ProfileController extends Controller
{
    ...
    public function index() 
    {
        ...
        return view('admin.profile.index',compact('u','country','state','city','sub_city','gender'));
    ...
    public function editPostProfile(Request $request) {
	 ...
	 $checkUser = User::where('email',$request->email)->where('id','!=',$id)->first();
        ...
        if(isset($checkUser->id) && $checkUser->id >0){
		    ...
		    return  redirect('admin/profile')->with(['error' => 'Sorry email address already exists.']);
        }
		$checkUser = User::where('mobile',$request->mobile)->where('id','!=',$id)->first();
		if(isset($checkUser->id) && $checkUser->id >0){
         
		    return  redirect('admin/profile')->with(['error' => 'Sorry mobile number already exists.']);
        }
