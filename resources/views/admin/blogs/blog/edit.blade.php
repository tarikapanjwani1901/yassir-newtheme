                       ...
                       <div class="row p-0">
                                	<div class="col-md-9">
                                         ...
                                         <label class="control-label">Title<span class="required_field">*</span> </label>    
                                          <input id="title" name="title" value="{{$blog->title}}" type="text" class="form-control required"  autocomplete="off">
                                    </div> 	
                                    </div>
                                    <div class="col-md-3">
                                   		<div class="form-group">
                                         <label class="control-label">Status<span class="required_field">*</span> </label>    
                                     	       {!! Form::select('status', $status, $blog->status,['class' => 'select2 form-control required']) !!}
                                    </div> 	
                                    </div>
                                    <div class="col-md-6">
                                   		<div class="form-group">
                                         ...
                                         <select name="category[]" multiple="multiple" class="form-control select2">
                                         	@foreach($category as $singleKey=>$single)
                                            	@if(in_array($singleKey,$categoryAssingList))
                                                <option value="{{$singleKey}}" selected="selected">{{$single}}</option>
                                                @else
                                                
                                                <option value="{{$singleKey}}">{{$single}}</option>
                                                @endif
                                            @endforeach
                                         </select>
                                	...
                                	<div class="col-md-6">
                                         ...
                                         <select name="tag[]" multiple="multiple" class="form-control select2">
                                         	@foreach($tags as $singleKey=>$single)
                                            	@if(in_array($singleKey,$tagAssingList))
                                                <option value="{{$singleKey}}" selected="selected">{{$single}}</option>
                                                @else
                                    ...
                                    <div class="col-md-12">
                                         ...
                                         <label class="control-label">Description</label>    
                                            ...
                                            <textarea rows='10' name="description" id="description" class='textarea form-control' style="width: 100%; height: 900px !important; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{$blog->content}}</textarea
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="fileinput-preview thumbnail" style="max-width: 200px; max-height: 200px;">
                                           	  @if($blog->image)
                                                        @if((substr($blog->image, 0,5)) == 'https')
                                                            <img src="{{ $blog->image }}" alt="img" class="img-responsive"/>
                                                        @else
                                                            <img src="{!! url('/').'/images/blog/'.$blog->id .'/'. $blog->image!!}" alt="img" class="img-responsive"/>
                                                        @endif
                                                    @endif
                                           </div>
                                        <br />
